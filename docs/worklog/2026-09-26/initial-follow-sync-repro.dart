...
class Log { static void logPrint(Object? value) { print(value); } }
class FollowUser {
  final String id, roomId, siteId, userName, face;
  final DateTime addTime;
  FollowUser({required this.id, required this.roomId, required this.siteId,
    required this.userName, required this.face, required this.addTime});
}
class History {
  final Map<String, dynamic> data;
  History.fromJson(this.data);
  Map<String, dynamic> toJson() => data;
}

Future<void> main() async {
  final dir = await Directory.systemTemp.createTemp('alllive-sync-proof-');
  try {
    FollowSyncService.baseDir = dir.path;
    FollowSyncService.isSubWindow = true;
    await FollowSyncService.syncFollow(FollowUser(id: 'huya_123', roomId: '123',
      siteId: 'huya', userName: 'test', face: '', addTime: DateTime(2026)));
    final child = <String>[];
    final parent = <String>[];
    await FollowSyncService.applyFollowEvents(onAdd: (v) async {child.add(v.id);},
      onRemove: (v) async {});
    FollowSyncService.isSubWindow = false;
    await FollowSyncService.applyFollowEvents(onAdd: (v) async {parent.add(v.id);},
      onRemove: (v) async {});
    if (child.length != 1 || parent.isNotEmpty) throw StateError('Unexpected result');
    print('PROVEN: subwindow consumed follow event; parent received ${parent.length}');

    FollowSyncService.isSubWindow = true;
    await FollowSyncService.syncSetting('PlayerVolume', 10);
    FollowSyncService.isSubWindow = false;
    final applied = <int>[];
    await FollowSyncService.applySettingEvents(onSet: (key, value) {
      applied.add(value as int);
      // Deterministically inject another writer after read and before rename.
      File(p.join(dir.path, 'subwindow_sync', 'settings', 'PlayerVolume.json'))
        .writeAsStringSync(jsonEncode({'key': key, 'value': 90, 'ts': 2}));
    });
    await FollowSyncService.applySettingEvents(onSet: (key, value) {
      applied.add(value as int);
    });
    final processed = jsonDecode(await File(p.join(dir.path,
      'subwindow_sync', 'settings', 'PlayerVolume.json.processed')).readAsString());
    if (applied.length != 1 || applied.single != 10 || processed['value'] != 90) {
      throw StateError('Unexpected race result');
    }
    print('PROVEN: concurrent settings update lost; applied=$applied; archived=${processed['value']}');
  } finally {
